// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.wpilibj2.command.CommandBase;
import frc.robot.subsystems.SpringAssemblyShooterSubsystem;

public class SpringAssemblyShooterCommand extends CommandBase {
  /** Creates a new SpringAssemblyShooterCommand. */
  SpringAssemblyShooterSubsystem shooter;
  public SpringAssemblyShooterCommand(SpringAssemblyShooterSubsystem subsystem) {
    // Use addRequirements() here to declare subsystem dependencies.
    this.shooter = subsystem;
    addRequirements(subsystem);
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
    shooter.outputLeft.getEncoder().setPosition(0);
    shooter.outputRight.getEncoder().setPosition(0);
    System.out.println("Started shooting");
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    shooter.shooting();
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {
    System.out.println("Stopped shooting");

  }

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    return false;
  }
}
